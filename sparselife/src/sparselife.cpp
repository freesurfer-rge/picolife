...
namespace SparseLife
{
    ...
    void SparseLife::AddCell(const Cell cell)
    {
        this->activeCells->emplace(cell);
    }

    void SparseLife::AddCells(const std::set<Cell> &cells)
    {
    ...
    std::unique_ptr<std::set<Cell>>
    SparseLife::ApplyRules(const std::set<Cell> &cellGrid) const
    {
    ...
    const std::set<Cell> &SparseLife::GetCells() const
    {
